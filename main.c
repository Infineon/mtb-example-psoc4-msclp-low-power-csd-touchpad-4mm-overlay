...
/* Active mode Scan time calculated in us ~= 287us */
#define ACTIVE_MODE_FRAME_SCAN_TIME     (287u)

/* Active mode Processing time in us ~= 85us */
#define ACTIVE_MODE_PROCESS_TIME        (85u)

/* ALR mode Scan time calculated in us ~= 287us */
#define ALR_MODE_FRAME_SCAN_TIME        (287u)

/* ALR mode Processing time in us ~= 85us */
#define ALR_MODE_PROCESS_TIME           (85u)
 ...
 ********************************************************************************
 * Summary:
 * Logic to control the on / off status with change in  brightness of LED2 and LED3
...
void led_control()
{
    ...
    /*******************************************************************************
     * If the CSD Touchpad is active, Turn On LED2 and LED3
    ...
    if (SENSOR_ACTIVE == Cy_CapSense_IsWidgetActive(CY_CAPSENSE_TOUCHPAD_SELF_CAP_WDGT_ID, &cy_capsense_context))
    {
        ...
        Cy_TCPWM_PWM_SetCompare0(CYBSP_PWM_0_HW, CYBSP_PWM_0_NUM, (MAXIMUM_BRIGHTNESS_LED - touchposition_x));


        /* LED2 Turns ON and brightness ncreases when the finger is swiped from bottom to top */
